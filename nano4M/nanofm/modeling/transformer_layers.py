...
class Attention(nn.Module):
    ...
    def __init__(self, dim: int, head_dim: int = 64, qkv_bias: bool = False, proj_bias: bool = False):
        ...
        # Single combined QKV projection: maps dim -> 3*dim
    ...
    def forward(self, x: torch.Tensor, mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        # Compute Q, K, V via the combined projection, then split into heads
        ...
        # Scaled dot-product attention matrix [B, num_heads, L, L]
        attn = (q @ k.transpose(-2, -1)) * self.scale

        if mask is not None:
            mask = rearrange(mask, "b n m -> b 1 n m") # Unsqueeze for multi-head attention
            # Wherever mask is False, set to -inf so softmax gives zero weight
            attn = attn.masked_fill(~mask, float('-inf'))

        # Softmax over last dimension
        attn = F.softmax(attn, dim=-1)

        # Weight values by attention and reshape back to [B, L, D]
...
class CrossAttention(nn.Module):
    ...
    def __init__(self, dim: int, head_dim: int = 64, qkv_bias: bool = False, proj_bias: bool = False):
        ...
        # Separate Q projection (from x) and combined KV projection (from context)
    ...
    def forward(self, x: torch.Tensor, context: torch.Tensor, mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        # Compute Q from x → [B, num_heads, N, head_dim]
        q = self.q_proj(x)
        q = rearrange(q, 'b n (h d) -> b h n d', h=self.num_heads)

        # Compute K, V from context → each [B, num_heads, M, head_dim]
        ...
        if mask is not None:
            mask = rearrange(mask, "b n m -> b 1 n m") # Unsqueeze for multi-head attention
            # Wherever mask is False, set to -inf
            attn = attn.masked_fill(~mask, float('-inf'))

        # Softmax over last dimension
        attn = F.softmax(attn, dim=-1)

        # Weight values and reshape to [B, N, D]
...
class DecoderBlock(nn.Module):
    ...
    def forward(self, 
        ...
        # Cross-attention with residual (query from x, key/value from context)
