...
class FourM(nn.Module):
    ...
    def forward_encoder(
        ...
        # Embed the input tokens using the input embedding layer. Shape: [B, N, D]
        x = self.enc_tok_emb(enc_input_tokens)

        # Embed the input modality IDs and sum to the token embeddings. Shape: [B, N, D]
        x = x + self.enc_mod_emb(enc_input_modalities)

        # Get the positional embeddings for the input positions and add them. Shape: [B, N, D]
    ...
    def forward_decoder(
        ...
        # Embed the target modality IDs using the embedding layer. Shape: [B, M, D]
        x = self.dec_mod_emb(dec_input_modalities)

        # Get the positional embeddings for the target positions and add them. Shape: [B, M, D]
        ...
        # Project context to the decoder dimension. Shape: [B, N, D]
        context = self.dec_context_proj(enc_context)

        # Add the encoder positional embeddings. Shape: [B, N, D]
    ...
    def forward_model(
        ...
        # Pass dec_x through linear output head to compute the logits. Shape: [B, M, vocab_size]
    ...
    def generate_one_modality_roar(
        ...
        # Randomly shuffle positions 0 to n_tokens_target - 1 and split into num_steps steps
