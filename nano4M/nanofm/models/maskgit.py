...
from nanofm.modeling.transformer_layers import LayerNorm, TransformerTrunk
...
class MaskGIT(nn.Module):
    ...
    def forward_model(self, x: torch.LongTensor, mask: torch.BoolTensor) -> torch.Tensor:
        ...
        h[mask] = self.mask_token

        h = h + self.positional_embedding[:L]

        h = self.trunk(h)

        logits = self.to_logits(self.out_norm(h))
    ...
    def generate_random_mask(self, seq: torch.Tensor) -> torch.BoolTensor:
        ...
        num_masked = torch.randint(1, L + 1, (B, 1), device=seq.device)
