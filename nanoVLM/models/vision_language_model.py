...
class VisionLanguageModel(nn.Module):
    ...
    def forward(self, input_ids, image, attention_mask=None, targets=None):
        ...
        if attention_mask is not None:
            image_attention = torch.ones(image.size(0), image_embeds.size(1), device=image.device)
    ...
    def generate(self, input_ids, image, attention_mask=None, max_new_tokens=5):
        ...
        if attention_mask is not None:
            image_attention_mask = torch.ones((batch_size, img_seq_len), device=input_ids.device)
        ...
        for i in range(max_new_tokens):
            ...
            if attention_mask is not None:
                attention_mask = torch.cat([attention_mask, torch.ones((batch_size, 1), device=input_ids.device)], dim=1)  # (vi)
    ...
    def generate_with_kv_cache(self, input_ids, image, attention_mask=None, max_new_tokens=5):
        ...
        # Step 2: PREFILL
        model_out, past_key_values = self.decoder.forward_kv(combined_embd, past_key_values=None, attention_mask=attention_mask)
